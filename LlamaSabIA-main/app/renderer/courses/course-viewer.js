...
class CourseViewer {
  ...
  renderContent(sheet, chapterId, sheetId) {
    // Determinar URL de Jupyter según el capítulo (Sprint/Proyecto abre el notebook específico)
    let jupyterUrl = 'http://localhost:8888';
    if (chapterId === 2) {
      jupyterUrl = 'http://localhost:8888/notebooks/PROYECTO_SPRINT1.ipynb';
    }
    ...
    const contentHtml = sheet.content.sections.map(section => {
      switch(section.type) {
        ...
        case 'html':
          // Sección HTML embebible con botón de Jupyter
          return `
            <div class="content-html">
              <div class="jupyter-header" style="margin-bottom: 16px;">
                <button onclick="window.electronAPI.openExternal('${jupyterUrl}')" 
                  ...
                  🚀 Abrir en Jupyter Notebook
                </button>
              </div>
              <iframe src="${section.src}" style="width:100%;height:720px;border:none;"></iframe>
            </div>
          `;
        case 'image':
          return `
    ...
    // Marcar como completada (evitar auto-completar la sección de Proyecto)
    // Si es el capítulo de Proyecto (id === 2) no hacemos auto-complete,
    // para que el usuario pueda ver y abrir el notebook/interactivo.
    if (chapterId !== 2) {
  ...
  checkAnswer() {
    ...
    let userAnswer;
      // Verificar si es el proyecto y se completaron las hojas anteriores
      if (chapterId === 2 && this.progress[this.currentCourse.id]?.completedSheets?.length >= 4) {
        return true;
      }
    
      // La primera hoja o hojas marcadas como unlocked están desbloqueadas
      if (sheet.unlocked || sheetId === 1) return true;
